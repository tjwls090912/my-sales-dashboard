...
# A 등급의 성과급률을 B 등급(4.0)보다 높은 5.0으로 설정하여 논리적 일관성을 확보합니다.
...
def average_sales(branch):
    """지점 평균."""
    # len(branch)는 4(지점명 포함)이므로 분기 수인 3으로 직접 나누어 평균 계산
...
def quarter_average(branches, quarter):
    ...
    total = 0
    for b in branches:
        # "1분기" 하드코딩 대신 매개변수로 받은 quarter 사용
        total += b[quarter]
...
def rank_list(branches):
    """총매출 기준 정렬."""
    # 순위 지정을 위해 내림차순(reverse=True)으로 정렬
